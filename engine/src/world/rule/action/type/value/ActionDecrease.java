...
public class ActionDecrease extends ActionValue {
    ...
    public ActionDecrease(PRDAction prdObject, Context context) throws Exception {
        super(prdObject, context);

        if(!context.getPrimaryEntityDefinition().getProperties().get(propertyName).isNumeric())
            throw new IllegalArgumentException("Property '" + propertyName + "' referenced to property in Decrease is not of a numeric type.");
        ...
        if(value instanceof EntityPropertyExpression){
            if(!context.getPrimaryEntityDefinition().getProperties().get(value.toString()).isNumeric())
                throw new IllegalArgumentException("Property '" + value + "' referenced to value in Decrease is not of a numeric type.");
        }
    }
